...
---@alias minx.kit.Vim.Keymap.Keys { keys: string, remap: boolean }
---@alias minx.kit.Vim.Keymap.KeysSpecifier string|{ keys: string, remap: boolean }

---@param keys minx.kit.Vim.Keymap.KeysSpecifier
---@return minx.kit.Vim.Keymap.Keys
...
---Send keys.
---@param keys minx.kit.Vim.Keymap.KeysSpecifier|minx.kit.Vim.Keymap.KeysSpecifier[]
...
function Keymap.send(keys, no_insert)
  ...
  return Async.new(function(resolve, _)
    ...
    local callback = Keymap.termcodes(('<Cmd>lua require("minx.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
    if no_insert then
